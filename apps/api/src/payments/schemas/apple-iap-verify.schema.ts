...
export const AppleVerifyRoomUnlockSchema = z.object({
  ...
  /** Apple transactionId for this specific purchase — used for idempotency and verification. */
...
export const AppleVerifyStorageSchema = z.object({
  roomId: z.string().min(1, 'roomId is required'),
  /** Apple transactionId for this subscription purchase — used for idempotency and verification. */
  transactionId: z.string().min(1, 'transactionId is required'),
});
